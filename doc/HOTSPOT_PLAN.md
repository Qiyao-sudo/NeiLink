...
### 2.1 状态持久化
- [x] 保存热点配置到设置（`SystemSettings` 新增 `hotspotSsid`、`hotspotPassword`、`hotspotRandomPassword` 字段）
- [x] 主页和设置页共享热点配置，统一持久化到 `settings.json`
- [x] 热点关闭状态下仍显示已保存的配置（后端 `getHotspotStatus` 返回 `currentConfig`）
...
### 2.3 安全性
- [x] 随机密码开关（开启时自动生成随机密码，关闭时使用自定义密码）
- [x] 自动生成安全密码

### 2.4 设置页面集成
- [x] 设置页新增热点名称（SSID）配置项
- [x] 设置页新增随机密码开关
- [x] 设置页新增自定义密码输入框（关闭随机密码时显示）
- [x] 设置页打开时自动同步主页热点配置
- [x] 设置页保存时若热点开启则停止并重启以应用新配置
- [x] 移除旧有的"热点前缀"和"热点密码长度"配置项

### 2.5 热点配置即时更新
- [x] 热点开启时保存配置自动先停止再启动以立即应用
- [x] 修复关闭随机密码后密码仍刷新为随机密码的问题
- [x] 修复重复弹出"热点配置已更新"提示的问题
...
### 2026-05-30: 阶段 2 完成
- 随机密码开关（主页 + 设置页）
- 热点配置统一持久化到 `settings.json`（`hotspotSsid`、`hotspotPassword`、`hotspotRandomPassword`）
- 设置页集成热点配置，与主页双向同步
- 热点开启时保存配置立即生效（先停止再启动）
- 修复热点关闭时配置显示为空的问题
- 修复关闭随机密码后密码仍刷新为随机密码的问题
- 修复重复弹出"热点配置已更新"提示的问题
- 移除旧有的"热点前缀"和"热点密码长度"配置项
- 修复托盘图标不显示的问题

### 2026-05-13: 阶段 1 完成
- 实现 Windows PowerShell + WinRT 命令管理热点
